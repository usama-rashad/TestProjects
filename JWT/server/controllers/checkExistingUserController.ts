...
export const checkExistingUserController = async (req: Request, res: Response) => {
  ...
  if (username == "" || password == "") {
    return res.status(200).json({ message: `No username or password entered. Try again.`, error: 1 });
  }
      ...
      checkUserPassword(password, userfound.password)
        .then((result) => {
          if (result) {
            ...
            // Password incorrect
            return res.status(200).json({ message: `User ${username} password incorrect. Check your password`, error: 1 });
          }
        })
        .catch((err) => {
          // System error
          return res.status(200).json({ message: "System error while checking for password.", error: 1 });
        });
    } else {
      // User not found
      return res.status(200).json({ message: `User ${username} not found.`, error: 1 });
    }
  });
};
